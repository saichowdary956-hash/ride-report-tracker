...
@app.post("/rows/update-all")
def rows_update_all():
    vehicle = request_vehicle()
    edited_rows = rows_from_bulk_form(form_lists(), vehicle)
    touched_sources = set()
    ...
    rebuild_tracker_from_database(OUTPUT_DIR, tracker_name=tracker_path_for_vehicle(vehicle).name, vehicle=vehicle)
    return render_app(
        f"Updated {len(edited_rows)} row(s). The Excel download has been regenerated with your latest edits.",
